...
void main(int argc, char **argv)
{
  ...
  myrange = findmyrange(nbodynum,nnodes,my_rank);
   ...
   // MPI_Bcast(&nbodynum, 1, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  for(z=0;z<chunksize;z++){
    MPI_Scatter(&b,chunksize,MPI_INT,&sub_b,chunksize,MPI_INT,0,MPI_COMM_WORLD);
    NbodyCalc(b,nbodynum,sub_b,chunksize);
    float *sub_body
    if (my_rank == 0){
      sub_bodies = malloc(sizeof(float) * chunksize);
    }
    MPI_Gather(&sub_body, 1, MPI_FLOAT, sub_bodies, 1, MPI_FLOAT, 0,
           MPI_COMM_WORLD);
  }
